                                    ...
                                    <td class="dashboard_propert_wrapper">
                                        ...
                                        <div class="title">
                                            ...
                                            <c:choose>
                                                ...
                                                <c:when test="${c.status == 2}"><span
                                                        class="text-warning">退租申请中</span></c:when>
                                                <c:when test="${c.status == 3}"><span
                                                        class="text-warning">退租失败</span></c:when>
                                            </c:choose>
                                        </div>
                                    </td>
                                    <td>${c.customerUser.userDisplayName}<br/>${c.customerUser.phone}</td>
                                    <td>${c.ownerUser.userDisplayName}<br/>${c.ownerUser.phone}</td>
                                    <td>
                                        ...
                                        <c:if test="${c.status == 2 &&(sessionScope.user.role == 'owner')}">
                                            <a href="javascript:void(0)" onclick="endOrderPass(${c.id})">通过退租申请</a><br/>
                                            <a href="javascript:void(0)" onclick="endOrderReject(${c.id})">拒绝退租申请</a>
                                        </c:if>
                                    </td>
                                </tr>
                            </c:forEach>
